	...
	return map[string]any{
		...
		"components": map[string]any{
			"schemas": map[string]any{
				...
				"DocumentRequest": object(map[string]any{
					"columns":  map[string]any{"type": "integer", "description": "Line width in characters; 0/omitted derives from the head width (80mm=48, 58mm=32)."},
					"elements": map[string]any{"type": "array", "items": ref("DocElement"), "description": "Ordered layout elements rendered top to bottom into one job."},
					"feed":     map[string]any{"type": "integer", "minimum": 0},
					"cut":      cut,
				}, []string{"elements"}),
				"DocElement": object(map[string]any{
					"type":         map[string]any{"type": "string", "enum": []string{"text", "row", "columns", "table", "rule", "feed", "qr", "barcode", "image"}, "description": "Element type; the fields below apply per type."},
					"text":         map[string]any{"type": "string", "description": "text: content (wrapped to the line width at default size; \\n forces breaks)."},
					"align":        align,
					"bold":         map[string]any{"type": "boolean"},
					"underline":    map[string]any{"type": "integer", "enum": []int{0, 1, 2}},
					"width":        map[string]any{"type": "integer", "minimum": 1, "maximum": 8, "description": "text: width size magnification."},
					"height":       map[string]any{"type": "integer", "minimum": 1, "maximum": 8, "description": "text: height size magnification."},
					"left":         map[string]any{"type": "string", "description": "row: left/label text."},
					"right":        map[string]any{"type": "string", "description": "row: right/value text, flush to the right edge."},
					"char":         map[string]any{"type": "string", "description": "rule: single fill character (default '-')."},
					"lines":        map[string]any{"type": "integer", "description": "feed: number of blank lines (default 1)."},
					"columns":      map[string]any{"type": "array", "items": ref("DocColumn"), "description": "columns/table: per-column width and alignment; omit for equal auto columns."},
					"cells":        map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "columns: one row of cell texts."},
					"rows":         map[string]any{"type": "array", "items": map[string]any{"type": "array", "items": map[string]any{"type": "string"}}, "description": "table: rows of cell texts."},
					"gap":          map[string]any{"type": "integer", "description": "columns/table: blank cells between columns (default 1)."},
					"data":         map[string]any{"type": "string", "description": "qr/barcode: content to encode."},
					"scale":        map[string]any{"type": "integer", "description": "qr: module pixel size (default 8)."},
					"recovery":     map[string]any{"type": "string", "enum": []string{"low", "medium", "high", "highest"}},
					"caption":      map[string]any{"type": "string", "description": "qr/barcode: text printed under the code."},
					"format":       map[string]any{"type": "string", "description": "barcode: symbology (default code128)."},
					"wide":         map[string]any{"type": "boolean", "description": "barcode: larger modules for dense codes."},
					"hri":          map[string]any{"type": "boolean", "description": "barcode: print the human-readable number under the code."},
					"image_base64": map[string]any{"type": "string", "contentEncoding": "base64", "description": "image: base64 PNG/JPEG/GIF, scaled to fit the head."},
				}, []string{"type"}),
				"DocColumn": object(map[string]any{
